...
|---|---|---|
| 发现并进入面板 | 随机访问路径、HTTPS、可选 BasicAuth、独立面板登录、失败限速与自动封禁、恶意扫描识别，以及满足条件时启用的 IPv4 nftables 联动 | 突破入口或拿到普通后台会话，仍不能直接读取已保存的敏感凭据 |
...
4. **绕过封禁检查**：封禁记录始终保存在面板数据库中，已封禁来源访问管理面板路径时会在应用层收到 `403`。如果系统存在可用的 `nft` 命令、面板进程有权限管理防火墙，并且 `sp_filter` 表、IPv4 集合及丢弃规则均创建成功，封禁的 IPv4 来源还会在面板 TCP 端口被 nftables 直接丢弃。
...
> **nftables 部署边界：** Debian 13 软件仓库提供 nftables，常规完整安装中也可能已经存在，但最小化系统、容器和部分云服务器镜像不保证预装。Server Panel 当前一键安装脚本不会安装或启用 nftables，只会为已经启用的 UFW 或 firewalld 放行面板端口。因此不能把 nftables 网络层拦截视为所有安装环境的默认能力。当前面板创建的是 IPv4 封禁集合；IPv6 访问仍依赖外部防火墙策略和面板自身的认证/应用层检查。
