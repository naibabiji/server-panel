...
项目采用 Go + SQLite，单二进制部署，提供中英文界面、内置 HTTPS、分层登录保护和指定敏感凭据加密。无论你在寻找 VPS 管理面板、服务器资产管理系统、网站到期提醒工具，还是轻量级 Linux 服务器监控面板，都可以用 Server Panel 管理自己的基础设施，而不必把密码和客户资料交给第三方 SaaS。
...
|---|---|---|
| 发现并进入面板 | 随机访问路径、HTTPS、可选 BasicAuth、独立面板登录、失败限速与自动封禁、恶意扫描识别，以及 nftables IPv4/IPv6 联动 | 突破入口或拿到普通后台会话，仍不能直接读取受查看密码保护的服务器/网站密码和服务商私密备注 |
...
- **安全凭据存储**：服务器 SSH 密码、面板密码、网站面板密码和服务商私密备注加密保存；Agent 密钥仅在生成时显示一次，数据库保存 SHA-256 哈希。
- **文件与本地存储**：在受限根目录中上传、下载、复制、移动、重命名、压缩和解压文件；查看、挂载、卸载或初始化本机数据盘。
- **备份与恢复**：手动或定时生成包含 SQLite 数据库和密钥的自包含加密备份，支持保留数量、邮件发送和跨面板上传恢复。
- **更新与系统维护**：检查及安装面板更新，支持自动更新、签名校验、健康检查、二进制回滚和有条件的数据库回滚；可检查并更新 Debian/Ubuntu 系统包。
...
登录信息会显示在安装日志中。安装脚本会为已启用的 UFW 或 firewalld 放行 HTTPS 端口；如果云厂商还配置了安全组，请在控制台手动放行对应 TCP 端口（默认 `8444`）。在线安装器目前不校验下载二进制的校验和或签名；请在可信网络使用，或改用自行核验 Release 资产后的离线安装。
...
- 面板更新包含下载、签名/校验和验证、数据库及二进制备份、替换、重启、健康检查、二进制回滚和有条件的数据库回滚。
- 可配置每日或每周自动备份、备份保留数量，以及在文件大小允许时通过 SMTP 发送备份。
- 新备份使用 `.spbackup` 加密格式；可上传到任意支持该格式的 Server Panel，并使用生成备份时的查看密码恢复。旧版 `.tar.gz` 仅保留兼容恢复。
- 面板备份只覆盖 SQLite 数据库与 `secret.key`，不包含 `config.json`、TLS 证书、systemd 配置或任意数据目录文件。
- 备份快照会清空 `metrics` 和 `host_metrics` 表中的监控历史；恢复后从新采集的数据开始。
...
4. **绕过封禁检查**：封禁记录始终保存在面板数据库中，已封禁来源访问管理面板路径时会在应用层收到 `403`。如果系统存在可用的 `nft` 命令、面板进程有权限管理防火墙，并且 `sp_filter` 表、IPv4/IPv6 集合及丢弃规则均创建成功，封禁来源还会在面板 TCP 端口被 nftables 直接丢弃。初始化任何一步失败时，面板不会误报网络层防护已经启用。
...
> **nftables 部署边界：** 以主要目标环境 Debian 13 为例，一键安装脚本会通过 APT 安装 nftables，面板启动时再创建独立的 IPv4/IPv6 封禁集合和面板端口丢弃规则。DNF/YUM 系统也会尝试安装 nftables；若发行版仓库不提供该软件包，安装会继续并明确告警，此时仍保留认证和应用层封禁，但需要管理员使用系统防火墙或云安全组补充网络层保护。安装脚本不会替用户启用一套全局默认拒绝策略，只管理面板自己的封禁表，并为已经启用的 UFW 或 firewalld 放行面板端口。
...
5. **暴力破解止损**：同一来源连续输错查看密码 5 次后，系统会清空已保存的服务器 SSH 密码、服务器面板密码和网站面板密码副本，并吊销查看令牌；服务商私密备注和其他业务记录仍保留。使用确认短语强制重置查看密码时，还会清除服务商私密备注并生成新的备份身份。
6. **Agent 隔离**：每台 Agent 使用独立密钥；重新生成后旧密钥失效。Agent 只上报监控数据，不会读取面板保存的密码。

这意味着：**攻击者即使窃取普通面板账号或一个已登录会话，也不能直接导出上述受保护凭据。** 他还必须在触发清空机制前得到独立查看密码，并满足令牌、会话、来源 IP 和有效期约束。SMTP 密码是例外：它当前以明文保存在 `settings` 表，并由已登录的 SMTP 设置接口返回，不受查看密码保护。
...
新生成的 `.spbackup` 会加密数据库与 `secret.key`，私有备份身份也只以查看密码加密后的形式写入文件。仅取得备份文件不能直接读取其中的敏感凭据；恢复时必须输入生成该文件时使用的查看密码。仍应使用强查看密码并妥善保管异地副本，因为攻击者可以对被盗文件进行离线密码猜测。历史 `.tar.gz` 备份未加密，应继续按明文敏感资产管理或安全销毁。

面板内更新失败时会尝试回滚二进制；数据库只会在服务已停止且有限的核心表可读性检查判定数据库损坏时回滚。因此数据库迁移必须保持旧版二进制兼容，不能把“失败回滚”理解为无条件恢复数据库。

> 自托管不等于自动安全。建议使用强密码、HTTPS，并按需要把面板放在云防火墙、VPN、Tailscale、WireGuard、Cloudflare 或可信反向代理之后。

> 使用自签面板证书生成 Agent 安装命令时，Agent 可能启用 `tls_skip_verify`；安装器只用同一下载来源提供的 SHA-256 校验值，不验证 Ed25519 签名。对抗主动网络攻击时，应使用可信证书和可信下载链路。
...
已有安装再次运行安装脚本时，会保留现有配置和数据，并替换程序、刷新服务/防火墙设置；它不会像面板内更新那样自动备份、健康检查或失败回滚。若要重新生成配置和登录信息：
...
旧版未加密备份可用命令行恢复；加密 `.spbackup` 请在任意支持该格式的 Server Panel 设置页上传并输入原查看密码：
