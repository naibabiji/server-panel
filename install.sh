...
print_summary() {
    ...
    echo ""
    echo "  管理命令:"
    echo "    systemctl status server-panel   # 查看服务运行状态"
    echo "    systemctl restart server-panel  # 重启服务（使手动改动的配置生效）"
    echo "    journalctl -u server-panel -f    # 实时跟踪服务日志"
    echo "    server-panel --reset-password    # 重置管理员密码（同时重置 BasicAuth 与面板登录，并自动重启服务）"
    echo "    server-panel --unban-all        # 清空所有封禁 IP 与登录失败记录（自己被误封时自救，立即生效无需重启）"
    echo ""
}
